...
router.post("/submitRegisterCredentials",function(req,res){
    User.register({username: req.body.username,name:req.body.name,birthDay:new Date(req.body.birthDay).toLocaleString(),image:false,privacy:"private"}, req.body.password, function(err, user){
      if (err) {
        console.log(err);
